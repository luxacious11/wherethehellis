...
async function FillTracker(username, params = {}) {
    ...
    const config = {
      ...
      historywrap: params.thisHistory || $('[data-clip="archived"]'),
      commwrap: params.thisCommTracker || $('[data-clip="comms"]'),
      commhistorywrap: params.thisCommHistoryTracker || $('[data-clip="archivedcomms"]'),
      socialhistorywrap: params.thisSocialHistoryTracker || $('[data-clip="archivedsocials"]'),
      socialwrap: params.thisSocialTracker || $('[data-clip="socials"]'),
      devwrap: params.thisDevTracker || $('[data-clip="dev"]'),
      reqwrap: params.thisDevTracker || $('[data-clip="requests"]'),
      eventwrap: params.thisEventTracker || $('[data-clip="events"]'),
    ...
    if (meta.length) {
      ...
      config.socialhistorywrap.append(`<div class="profile--nothreads">${boardmessage}</div>`);
      ...
      if (i > 0) {
        ...
        if (!config.ignoreforums.includes(location) && !config.ignoreforumids.includes(location_id)) {
          ...
          if (history) {
            ...
            config.socialhistorywrap.append(formatItem('archived', threadLink, title, threadDesc, location_id, location, lastPosterId, lastPoster, postDate));
          } else if (dev) {
